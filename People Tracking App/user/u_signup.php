...
if(isset($_REQUEST['submit'])){ // PHP Signup Validation Starts
   ...
   $repeat_password = $_REQUEST["cpassword"];
    ...
    if(strlen($email)<=0){
      ...
      //ID validation
      if(strlen($idtype)<=0){
        $idtypeHelp = "<small id='idtypeHelp' style='display:block;color:yellow;' class='form-text text-muted'>Make sure you fill valid ID type...!</small>";
        array_push($errorArray,"mobile");
        $valid = false;
      }
      if(strlen($linkedid)<=0){
        $linkedidHelp = "<small id='linkedidHelp' style='display:block;color:yellow;' class='form-text text-muted'>Make sure you fill valid ID Number...!</small>";
        array_push($errorArray,"mobile");
        $valid = false;
      }
       ...
       //  Mobile Validation
    if(strlen($email)<=0){
        $mobileHelp = "<small id='mobileHelp' style='display:block;color:yellow;' class='form-text text-muted'>Make sure you fill correct Mobile Number...!</small>";
        array_push($errorArray,"mobile");
        $valid = false;
      }else if(strlen($email)>0){
        // Checking For Duplicate Email Exists
        $checkMobile = "SELECT mobile FROM user WHERE mobile = '$mobile'";
        $result= mysqli_query($connection,$checkMobile);
        $mobileCount = mysqli_num_rows($result);
        if($mobileCount>0){
          array_push($errorArray,"mobile");
          $mobileHelp = "<small id='mobileHelp' style='display:block;color:yellow;' class='form-text text-muted'>This Mobie is already exists...!</small>";
          $valid = false;
        }
      }else{
        echo "Something Wrong";
     ...
     if(count($errorArray)<=0 || $valid == true){
        $checkKey = "SELECT user_key FROM user ORDER BY user_key DESC LIMIT 1";
        ...
        $user_id = "USE".$fetched_value;
        $encr_pwd = password_hash($password,PASSWORD_BCRYPT);
        $password = $encr_pwd;
        $email = strtolower($email);

        $insertUser = "INSERT INTO user(email,mobile,name,password,user_key,user_id,linked_id,linked_id_type)
        VALUES('$email','$mobile','$name','$password','$fetched_value','$user_id','$linkedid','$idtype')";
        $success = mysqli_query($connection,$insertUser);
        if ($success) {
...
?>
            <div class="row signupForm" style="margin-top:150px;width:80%">
                <div class="col-md-12 login-form-1" >
                    ...
                    <form method="POST" action="<?php $_SERVER["PHP_SELF"]?>">
                        ...
                        <div class="form-group" style="display:flex;justify-content:space-between;">
                            ...
                            <select name="countrycode" id="countryCode" style="color:black;">
                            <optgroup label="Other countries" style="color:black;">
                            ...
                            <input type="number" id="mobile" name="mobile" class="form-control" placeholder="Mobile"  required />
                      </select>
                            <?php if(isset($mobileHelp)){ echo $mobileHelp;}?>
                        </div>
                        
      
                        <div class="form-group">
                            <label for="password">ID Type:</label>
                            <select name="idtype" id="idtype" style="color:black;">
                              <option value="aadhar">Aadhar</option>
                              <option value="voter">Voter</option>
                              <option value="pan">Pan</option>
                            </select>
                            <?php if(isset($idtypeHelp)){ echo $idtypeHelp;}?>
                        </div>
                        <div class="form-group">
                            <label for="linkedId">Linked ID:</label>
                            <input type="text" id="linkedId" name="linkedid" class="form-control" placeholder="Please Enter Linked ID Number"  required />
                            <?php if(isset($linkedidHelp)){ echo $linkedidHelp;}?>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <a href="u_login.php" class="ForgetPwd">Login!</a>
                        </div>
                    </form>
                </div>
            </div>
